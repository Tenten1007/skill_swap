...
:root {
	...
	--bg-gradient: linear-gradient(-45deg, #0f0f23 0%, #1a1a2e 25%, #16213e 50%, #0f3460
		75%, #533483 100%);
...
.header-title h1 {
	...
	background: linear-gradient(135deg, var(--primary) 0%, var(--secondary)
		30%, var(--accent) 60%, var(--success) 100%);
	-webkit-background-clip: text;
	-webkit-text-fill-color: transparent;
	background-clip: text;
...
.stat-card {
	background: linear-gradient(135deg, rgba(255, 255, 255, 0.25) 0%,
		rgba(255, 255, 255, 0.10) 50%, rgba(255, 255, 255, 0.05) 100%);
	border: 2px solid rgba(255, 255, 255, 0.3);
	border-radius: var(--radius-xl);
	padding: var(--space-xl);
	backdrop-filter: blur(25px);
	box-shadow: 0 15px 35px rgba(0, 0, 0, 0.1), inset 0 1px 0
...
.stat-card:hover {
	...
	box-shadow: 0 25px 50px rgba(0, 0, 0, 0.15), 0 0 0 1px
		rgba(99, 102, 241, 0.3);
}
	...
	<nav class="modern-navbar">
		<div class="navbar-container">
			<a href="home" class="navbar-brand"> <i
				class="fas fa-exchange-alt"></i> Skill Swap
			</a>
			...
			<div class="navbar-menu" id="navbarMenu">
				<a href="home" class="navbar-link nav-home"> <i
					class="fas fa-home"></i> For You
				</a> <a href="jobboard" class="navbar-link nav-board"> <i
					class="fas fa-search"></i> Browse All
				</a> <a href="add-skill" class="navbar-link nav-add"> <i
					class="fas fa-plus-circle"></i> Add Skill
				</a> <a href="my-offers" class="navbar-link nav-offers"> <i
					class="fas fa-briefcase"></i> My Skills
				</a> <a href="matches" class="navbar-link nav-matches active"> <i
					class="fas fa-handshake"></i> Matches
				</a> <a href="profile" class="navbar-link nav-profile"> <i
	...
	<div class="matches-container">
		...
		<div class="content-container">
			...
			<div class="tabs-section">
				...
				<div class="tabs-nav">
					<a href="?tab=received"
						class="tab-button ${activeTab == 'received' ? 'active' : ''}">
						<i class="fas fa-inbox"></i> คำขอที่ได้รับ <c:if
							test="${pendingCount > 0}">
							<span
								style="background: var(--error); color: white; border-radius: 50%; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center; font-size: 0.75rem;">${pendingCount}</span>
						</c:if>
					</a> <a href="?tab=sent"
						class="tab-button ${activeTab == 'sent' ? 'active' : ''}"> <i
						class="fas fa-paper-plane"></i> คำขอที่ส่ง
					</a> <a href="?tab=learning"
						class="tab-button ${activeTab == 'learning' ? 'active' : ''}">
						<i class="fas fa-graduation-cap"></i> กำลังเรียน <c:if
							test="${learningCount > 0}">
							<span
								style="background: var(--success); color: white; border-radius: 50%; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center; font-size: 0.75rem;">${learningCount}</span>
						</c:if>
					</a>
				</div>
										...
										<div class="request-card">
												...
												<h3>
													...
													<c:choose>
														<c:when test="${request.requester.id != null}">
															<a href="user-profile?userId=${request.requester.id}"
																style="color: inherit; text-decoration: none; transition: color 0.3s;"
																onmouseover="this.style.color='var(--primary)'"
																onmouseout="this.style.color='inherit'">${request.requester.username}</a>
														</c:when>
														<c:otherwise>
											...
											<div class="request-meta">
												<span> <i class="fas fa-calendar"></i>
													${request.createdAt.dayOfMonth}
													${request.createdAt.month.toString().substring(0,3).toLowerCase()}
													${request.createdAt.year}
												</span>
												<c:if test="${request.respondedAt != null}">
													<span> <i class="fas fa-reply"></i> ตอบกลับ:
														${request.respondedAt.dayOfMonth}/${request.respondedAt.monthValue}
													</span>
												</c:if>
											</div>
											...
											<c:if test="${request.status == 'PENDING'}">
												<div class="request-actions">
													<form method="post" action="respond-swap"
							...
							<c:otherwise>
								<div class="empty-state">
									...
									<p class="empty-description">
										คุณยังไม่ได้รับคำขอแลกเปลี่ยนสกิลใด ๆ
										ลองไปโพสต์สกิลของคุณเพื่อให้คนอื่นสามารถส่งคำขอมาได้!</p>
									<a href="add-skill" class="action-button btn-primary"> <i
										class="fas fa-plus"></i> เพิ่ม Skill ของคุณ
									</a>
								</div>
							</c:otherwise>
						</c:choose>
						...
						<c:choose>
								...
								<div class="requests-grid">
									<c:forEach var="request" items="${sentRequests}">
										<div class="request-card">
												...
												<h3>
													...
													<c:choose>
														<c:when test="${request.requestedSkill.user.id != null}">
															<a
																href="user-profile?userId=${request.requestedSkill.user.id}"
																style="color: inherit; text-decoration: none; transition: color 0.3s;"
																onmouseover="this.style.color='var(--primary)'"
																onmouseout="this.style.color='inherit'">${request.requestedSkill.user.username}</a>
														</c:when>
														<c:otherwise>
											...
											<c:if test="${request.status == 'PENDING'}">
												<div class="request-actions">
													<form method="post" action="cancel-swap"
														style="display: inline;">
														<input type="hidden" name="requestId"
															value="${request.id}">
							...
							<c:otherwise>
								<div class="empty-state">
									...
									<p class="empty-description">
										คุณยังไม่ได้ส่งคำขอแลกเปลี่ยนสกิลใด ๆ
										ลองไปดูสกิลที่น่าสนใจในหน้าหลักแล้วส่งคำขอแลกเปลี่ยนกันเลย!</p>
									<a href="home" class="action-button btn-primary"> <i
										class="fas fa-search"></i> ค้นหา Skills
									</a>
								</div>
							</c:otherwise>
						</c:choose>
						...
						<c:choose>
								...
								<div class="requests-grid">
									<c:forEach var="match" items="${myLearning}">
										<div class="request-card">
												...
												<h3>
													<c:choose>
														<c:when test="${match.offerer.id == user.id}">
                                                            เรียนกับ
                                                            <a
																href="user-profile?userId=${match.requester.id}"
																style="color: inherit; text-decoration: none; transition: color 0.3s;"
																onmouseover="this.style.color='var(--primary)'"
																onmouseout="this.style.color='inherit'">${match.requester.username}</a>
														</c:when>
														<c:otherwise>
                                                            เรียนกับ
                                                            <a
																href="user-profile?userId=${match.offerer.id}"
																style="color: inherit; text-decoration: none; transition: color 0.3s;"
																onmouseover="this.style.color='var(--primary)'"
																onmouseout="this.style.color='inherit'">${match.offerer.username}</a>
														</c:otherwise>
													</c:choose>
												</h3>
												<div
													class="request-status status-${match.status.toLowerCase().replace('_', '_')}">
													<c:choose>
											...
											<div class="request-meta">
												<span> <i class="fas fa-calendar"></i> เริ่ม:
													${match.createdAt.dayOfMonth}
													${match.createdAt.month.toString().substring(0,3).toLowerCase()}
													${match.createdAt.year}
												</span>
												<c:if test="${match.updatedAt != null}">
													<span> <i class="fas fa-clock"></i> อัปเดต:
														${match.updatedAt.dayOfMonth}/${match.updatedAt.monthValue}
													</span>
												</c:if>
											</div>
											...
											<div class="request-actions">
												<c:choose>
													<c:when test="${match.status == 'MATCHED'}">
														<form method="post" action="start-learning"
															style="display: inline;">
													...
													<c:when test="${match.status == 'IN_PROGRESS'}">
														<form method="post" action="complete-learning"
															style="display: inline;">
															<input type="hidden" name="matchId" value="${match.id}">
													...
													<c:when test="${match.status == 'COMPLETED'}">
														<c:set var="partnerUserId"
															value="${match.offerer.id == user.id ? match.requester.id : match.offerer.id}" />
														<!-- ส่งทั้ง userId และ matchId -->
														<a
															href="${pageContext.request.contextPath}/rating/give?userId=${partnerUserId}&matchId=${match.id}"
															class="btn-action btn-review"> <i class="fas fa-star"></i>
															ให้คะแนน
														</a>
													</c:when>
												</c:choose>
							...
							<c:otherwise>
								<div class="empty-state">
									...
									<p class="empty-description">เมื่อคุณยอมรับคำขอแลกเปลี่ยน
										จะปรากฏที่นี่เพื่อให้คุณติดตามการเรียนรู้</p>
									<a href="?tab=received" class="action-button btn-primary">
										<i class="fas fa-inbox"></i> ดูคำขอที่ได้รับ
									</a>
								</div>
							</c:otherwise>
						</c:choose>
